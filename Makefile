...
up: packet_redirect.bpf.o
	...
	# |                     |veth22@1 10.0.2.20|--|veth22@2 10.0.2.11    |
	...
	$(EXEC) $(NET_1) ip link add name veth22@1 type veth peer name veth22@2 netns $(NET_2)
	...
	$(EXEC) $(NET_1) ip link set veth22@1 up
	$(EXEC) $(NET_2) ip link set veth2@2 up
	$(EXEC) $(NET_2) ip link set veth22@2 up
	...
	$(EXEC) $(NET_1) /bin/sh -c 'echo 1 > /proc/sys/net/ipv4/conf/all/proxy_arp'
	$(EXEC) $(NET_1) /bin/sh -c 'echo 1 > /proc/sys/net/ipv4/conf/all/forwarding'
